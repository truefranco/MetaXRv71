...
inline bool AddTriangle(FScreenTriangle& Tri, float TriDepth, FPrimitiveComponentId PrimitiveId, uint8 MeshFlags, FOcclusionFrameData& InData)
{
	if (MeshFlags == 1) // occluder tri
	{
		...
		if (Tri.V[0].Y > Tri.V[1].Y)
			Swap(Tri.V[0], Tri.V[1]);
		if (Tri.V[1].Y > Tri.V[2].Y)
...
static void ProcessOccludeeGeomSIMD(const FMatrix& InMat, const FVector* InMinMax, int32 Num, int32* RESTRICT OutQuads, float* RESTRICT OutQuadDepth, int32* RESTRICT OutQuadClipped)
{
	...
	for (int32 k = 0; k < Num; ++k)
	{
		...
		VectorRegister4Int IntMinMax = VectorFloatToInt(VectorCombineLow(vScreenMin, vScreenMax));
...
static bool ProcessOccludeeGeom(const FOcclusionSceneData& SceneData, FOcclusionFrameData& FrameData, TMap<FPrimitiveComponentId, bool>& VisibilityMap)
{
	...
	MS_ALIGN(SIMD_ALIGNMENT)
	int32 Quads[RUN_SIZE * 4] GCC_ALIGN(SIMD_ALIGNMENT);
...
public:
	...
	void AddElements(const FOccluderVertexArraySP& Vertices, const FOccluderIndexArraySP& Indices, const FMatrix& LocalToWorld)
	{
...
static int32 CollectOccluderElements(FPrimitiveSceneProxy* InProxy, class FSWOccluderElementsCollector& Collector)
{
	check(InProxy);
	auto AssetUserRenderDataArray = InProxy->GetAssetUserRenderData();
	if (!AssetUserRenderDataArray)
	{
		return 0;
	}

	FStaticMeshOccluderData* OccluderData = nullptr;
	for (FAssetUserRenderData* AssetUserRenderData : *AssetUserRenderDataArray)
	{
		OccluderData = Cast<FStaticMeshOccluderData>(AssetUserRenderData);
		// TODO: we only support ONE/FIRST occluder ATM
		if (OccluderData)
		{
			break;
		}
	...
	if (OccluderData)
	{
		FMatrix OccluderToOwner = FScaleMatrix::Make(OccluderData->OccluderMeshScale);
		OccluderToOwner.SetOrigin(OccluderData->OccluderMeshOffset);

		TArray<FMatrix> LocalToWorlds;
		InProxy->GetLocalToWorldArray(LocalToWorlds);
		for (auto& LocalToWorld : LocalToWorlds)
		{
			Collector.AddElements(OccluderData->VerticesSP, OccluderData->IndicesSP, OccluderToOwner * LocalToWorld);
		}
		return LocalToWorlds.Num();
	}
...
static FGraphEventRef SubmitScene(const FScene* Scene, FViewInfo& View, FOcclusionFrameResults* Results)
{
	...
	// Collect scene geometry: occluders, occludees
	{
		...
		for (const FPotentialOccluderPrimitive& PotentialOccluder : PotentialOccluders)
		{
			...
			if (bCanBeOccluder)
			{
				...
				NumCollectedOccluders += CollectOccluderElements(Proxy, Collector);
			}
	...
	return FFunctionGraphTask::CreateAndDispatchWhenReady([SceneDataParam = MoveTemp(SceneData), Results]() {
...
{
	if (TaskRef.IsValid() && FTaskGraphInterface::IsRunning())
	{
...
void FSceneSoftwareOcclusion::DebugDraw(FRDGBuilder& GraphBuilder, const FViewInfo& View, FScreenPassRenderTarget Output, int32 InX, int32 InY)
{
	...
	if (GSOVisualizeBuffer == 0 || !CVarMobileAllowCustomOcclusion || CVarMobileAllowCustomOcclusion->GetValueOnAnyThread() == 0)
	{
		return;
	}
