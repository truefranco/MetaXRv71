...
FOvrpLayer::~FOvrpLayer()
{
	if (IsInGameThread())
	{
		ExecuteOnRenderThread([OvrpLayerIdTmp = this->OvrpLayerId, DeferredDeletionTmp = this->DeferredDeletion]()
		{
			DeferredDeletionTmp->AddOVRPLayerToDeferredDeletionQueue(OvrpLayerIdTmp);
		});
	}
	else
	{
...
#else
	// PokeAHole is un-necessary on PC except PT due to depth buffer sharing and compositing.
	return IsPassthroughShape() && IsDepthEnabled();
...
bool FLayer::BuildPassthroughPokeActor(FOculusPassthroughMeshRef PassthroughMesh, FPassthroughPokeActor& OutPassthroughPokeActor)
{
	...
	FOculusHMD* OculusHMD = static_cast<FOculusHMD*>(GEngine->XRSystem->GetHMDDevice());
#if PLATFORM_WINDOWS
	UMaterial* PokeAHoleMaterial = OculusHMD->GetResourceHolder()->PokeAHoleInverseMaterial;
#else
	UMaterial* PokeAHoleMaterial = OculusHMD->GetResourceHolder()->PokeAHoleMaterial;
#endif
	UMaterialInstanceDynamic* DynamicMaterial = UMaterialInstanceDynamic::Create(PokeAHoleMaterial, NULL);
	...
	if (Desc.HasShape<FUserDefinedLayer>())
	{
		...
		if(NeedsPassthroughPokeAHole())
		{
			for (const FUserDefinedGeometryDesc& GeometryDesc : UserGeometryList )
			{
				...
				{
					OculusHMD::FOculusPassthroughMeshRef GeomPassthroughMesh = GeometryDesc.PassthroughMesh;
...
{
	return ((shape != ovrpShape_ReconstructionPassthrough) &&
...
bool FLayer::CanReuseResources(const FLayer* InLayer) const
{
	...
	if (OvrpLayerDesc.Shape == ovrpShape_EyeFov)
	{
		if (OvrpLayerDesc.EyeFov.DepthFormat != InLayer->OvrpLayerDesc.EyeFov.DepthFormat ||
			OvrpLayerDesc.EyeFov.MotionVectorDepthFormat != InLayer->OvrpLayerDesc.EyeFov.MotionVectorDepthFormat ||
			OvrpLayerDesc.EyeFov.MotionVectorFormat != InLayer->OvrpLayerDesc.EyeFov.MotionVectorFormat ||
			OvrpLayerDesc.EyeFov.MotionVectorTextureSize.w != InLayer->OvrpLayerDesc.EyeFov.MotionVectorTextureSize.w ||
			OvrpLayerDesc.EyeFov.MotionVectorTextureSize.h != InLayer->OvrpLayerDesc.EyeFov.MotionVectorTextureSize.h
			)
		{
			return false;
		}
					...
					// Left
					{
						...
						for (int32 TextureIndex = 0; TextureIndex < TextureCount; TextureIndex++)
						{
							...
							if (bValidMotionVectorTextures)
							{
								...
								if (!OVRP_SUCCESS(FOculusHMDModule::GetPluginWrapper().GetLayerTextureSpaceWarp(OvrpLayerId, TextureIndex, ovrpEye_Left, &MotionVectorTextures[TextureIndex], &MotionVectorTextureSize, &MotionVectorDepthTextures[TextureIndex], &MotionVectorDepthTextureSize)) ||
									MotionVectorTextures[TextureIndex] == (unsigned long long)nullptr)
								{
		...
		if(bLayerCreated)
		{
			...
			if (ShapeNeedsTextures(OvrpLayerDesc.Shape))
			{
				...
				if (PropagateAlpha == EAlphaChannelMode::AllowThroughTonemapper) 
				{
					...
					InvAlphaTexture = RHICreateTexture2D(SizeX, SizeY, ColorFormat, NumMips, NumSamples, ColorTexCreateFlags, Info);
					InvAlphaTexture->SetName(TEXT("InvAlphaTexture"));
				}
#endif
...
void GetTrackingSpaceDeltaPose(const FSettings* Settings, const FGameFrame* Frame, FTransform& TrackingSpaceDeltaPose)
{
	...
	// Define: OVRPlugin-Device-FPose: same space with the OvrPlugin returned Pose ( In FPose, not ovrpPose ). FOculusHMDModule::GetPluginWrapper().GetNodePoseState3() returned value in this space
...
void FLayer::AddPassthroughMesh_RenderThread(const TArray<FVector>& Vertices,const TArray<int32>& Triangles, FMatrix Transformation, uint64_t& OutMeshHandle, uint64_t& OutInstanceHandle)
{
	...
	if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().AddInsightPassthroughSurfaceGeometry(
		OvrpLayerId,
		MeshHandle,
		OvrTransformation,
		&InstanceHandle)))
...
void FLayer::UpdatePassthroughMeshTransform_RenderThread(uint64_t InstanceHandle,FMatrix Transformation)
{
	...
	if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().UpdateInsightPassthroughGeometryTransform(
		InstanceHandle,
	   OvrTransformation)))
	{
...
void FLayer::RemovePassthroughMesh_RenderThread(uint64_t MeshHandle, uint64_t InstanceHandle)
{
	...
	if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().DestroyInsightPassthroughGeometryInstance(InstanceHandle)))
	{
		UE_LOG(LogTemp, Error, TEXT("Failed removing passthrough surface from scene."));
		return;
	}

	if (OVRP_FAILURE(FOculusHMDModule::GetPluginWrapper().DestroyInsightTriangleMesh(MeshHandle)))
	{
