...
namespace OculusInput
{
	...
	{
		return ((IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().IsInitialized()));
	}
...
FQuat FOculusHandTracking::GetBoneRotation(const int32 ControllerIndex, const EOculusHandType DeviceHand, const EBone BoneId)
{
	...
	if (OculusHMD::FOculusHMD::GetOculusHMD() != nullptr)
	{
...
bool FOculusHandTracking::IsHandTrackingEnabled()
{
	...
	{
#if OCULUS_INPUT_SUPPORTED_PLATFORMS
		if (IOculusHMDModule::IsAvailable() && FOculusHMDModule::GetPluginWrapper().IsInitialized())
		{
			ovrpBool result;
...
void FOculusHandTracking::SetControllerDrivenHandPoses(const EOculusXRControllerDrivenHandPoseTypes Type)
{
	...
	case EOculusXRControllerDrivenHandPoseTypes::None:
		FOculusHMDModule::GetPluginWrapper().SetControllerDrivenHandPoses(false);
		FOculusHMDModule::GetPluginWrapper().SetControllerDrivenHandPosesAreNatural(false);
		break;
	case EOculusXRControllerDrivenHandPoseTypes::Natural:
		FOculusHMDModule::GetPluginWrapper().SetControllerDrivenHandPoses(true);
		FOculusHMDModule::GetPluginWrapper().SetControllerDrivenHandPosesAreNatural(true);
		break;
