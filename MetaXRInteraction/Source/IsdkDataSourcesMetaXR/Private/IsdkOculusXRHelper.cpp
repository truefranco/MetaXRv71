...
bool FIsdkOculusXRHelper::IsHoldingAController(UObject* WorldContextObject)
{
  const auto World =
      GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::ReturnNull);
  if (!World)
  {
    // Generally default to controller being held if something goes wrong.
    return true;
  }

  const auto MetaXrSubsystem = World->GetSubsystem<UIsdkDataSourcesMetaXRSubsystem>();
  if (!MetaXrSubsystem)
  {
    return true;
  }

  return MetaXrSubsystem->IsHoldingAController();
}
